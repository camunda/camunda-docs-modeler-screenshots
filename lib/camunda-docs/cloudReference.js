 ...
 * @param {string} displayVersion - The Modeler version to display in the footer (e.g., '8.7.0')
 * @returns {Array} Array of screenshot tasks
 */
function generateScreenshotsForVersion(versionPath, displayVersion) {
 ...
 * If displayVersion is provided, uses it for all versions (backward compatibility).
 * Otherwise, generates screenshots for each configured version with its appropriate Modeler version.
 */
module.exports = function startScreenshotBatch(displayVersion) {

  // If a specific display version is provided, use the old behavior (single version)
  if (displayVersion) {
    return generateScreenshotsForVersion('docs', displayVersion);
  }

  // Otherwise, generate screenshots for all configured versions
  const tasks = [];
  for (const versionConfig of CAMUNDA_DOCS_VERSIONS) {
    const versionTasks = generateScreenshotsForVersion(versionConfig.path, versionConfig.modelerVersion);
