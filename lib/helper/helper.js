...
async function copyPlugins(extractedPath) {
  const pluginsPath = path.join(__dirname, '../fixtures/plugins');

  const modelerPluginsPath = path.join(extractedPath, 'resources', 'plugins');

  if (!existsSync(modelerPluginsPath)) {
    await fs.mkdir(modelerPluginsPath, { recursive: true });
  }

  const pluginFolders = await fs.readdir(pluginsPath, { withFileTypes: true });

  for (const pluginFolder of pluginFolders) {
    if (pluginFolder.isDirectory()) {
      const sourcePath = path.join(pluginsPath, pluginFolder.name);
      const destinationPath = path.join(modelerPluginsPath, pluginFolder.name);

      console.log(`Copying plugin ${pluginFolder.name} to ${destinationPath}`);
...
module.exports.copyPlugins = copyPlugins;