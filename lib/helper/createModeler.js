...
const { getExecutablePath, fetchLatestModelerVersion } = require('./helper');
...
class Modeler {
  ...
  async open() {
    ...
    const version = process.env.VERSION || await fetchLatestModelerVersion();

    const executablePath = getExecutablePath(process.platform, version);
