...
class Modeler {
  ...
  async rightClick(selector) {
    await this.app.client.rightClick(selector);
  }
  ...
  async annotate(selector, text, options) {
    await this.app.webContents.executeJavaScript(addAnnotation.toString() +
    `; addAnnotation('${selector}', '${text}', ${JSON.stringify(options)});`);
  }

  async connectTwoElements(selector1, selector2) {
    await this.app.webContents.executeJavaScript(connectTwoElements.toString() +
    `; connectTwoElements('${selector1}', '${selector2}');`);
  }

  async clearAnnotation() {
    await this.app.webContents.executeJavaScript(clearAllAnnotations.toString() + '; clearAllAnnotations();');
  }
