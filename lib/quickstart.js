...
module.exports = async function startScreenshotBatch() {
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      await modeler.annotate('.djs-context-pad.open .bpmn-icon-task', 'Select activity shape (rectangle)', annotationOptions.right);
      await modeler.annotate('[data-element-id="StartEvent_1"]', 'Double-click on element to edit label\\n(Use SHIFT + ENTER for line break)', annotationOptions.top);
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      await modeler.annotate('.djs-context-pad.open', '1. Select activity\\n2. Click wrench icon and change type to Service Task', annotationOptions.right);
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      await modeler.annotate('[data-entry="id"]', 'Set Process Id', annotationOptions.left);
      await modeler.annotate('[data-entry="name"]', 'Set Process Name', annotationOptions.left);
      await modeler.annotate('[data-entry="process-is-executable"]', 'Mark the process to be executable', annotationOptions.left);
  ...
  await triggerScreenshot(
    'camunda-docs-static/get-started/content/quick-start/img/modeler-save-diagram.png',
    async (filepath) => {
      const diagramPaths = [ path.join(__dirname, './fixtures/bpmn/quickstart/step1.3.bpmn') ],
            config = path.join(__dirname, './fixtures/user-data/small_with_prop_panel.json');
      ...
      await modeler.mouseOver('.app-icon-save-as');
      await modeler.annotate('.app-icon-save-as', 'Save Diagram as...');
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      await modeler.mouseOver('.app-icon-deploy');
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      await modeler.app.client.waitForVisible('#deployment\\.name')
        ...
        .keys('Backspace')
        .setValue('#deployment\\.name', 'Payment Retrieval');
      await modeler.takeScreenshot(filepath);
    }
  );
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      modeler.annotate(
        ...
        'Select activity shape and drag\\ninto position between Start\\nEvent and \\"Charge Credit\\nCard\\" Service Task',
        annotationOptions.right
      );

      modeler.connectTwoElements('.SCREENSHOT__ANNOTATION', '[data-element-id="userTask"]');
  ...
  await triggerScreenshot(
    ...
    async (filepath) => {
      ...
      await modeler.click('[data-element-id="userTask"]');
      await modeler.click('#camunda-assignee');
  ...
  await triggerScreenshot('camunda-docs-static/get-started/content/quick-start/img/modeler-gateway1.png',
    async (filepath) => {
      const diagramPaths = [ path.join(__dirname, './fixtures/bpmn/quickstart/step3.1.bpmn') ],
            config = path.join(__dirname, './fixtures/user-data/wide_with_prop_panel.json');
